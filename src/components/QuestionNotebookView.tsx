...
export const QuestionNotebookView: React.FC<QuestionNotebookViewProps> = ({ onOpenQuickTheory }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      ...
      {/* Header Banner */}
      <div className="bg-gradient-to-r from-amber-600 via-orange-600 to-amber-700 rounded-3xl p-6 sm:p-8 text-white shadow-md flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-4 flex flex-wrap items-center justify-between gap-4 shadow-xs">
        ...
        <div className="text-xs text-slate-500 font-medium">
          Tổng cộng: <strong className="text-amber-700">{filteredBookmarks.length}</strong> câu hỏi đã lưu
        </div>
      </div>
      ...
      <div className="space-y-4">
        {filteredBookmarks.map(b => {
          ...
          return (
            <div
              key={b.id}
              className="bg-white rounded-2xl border border-slate-200/90 hover:border-amber-300 transition-all p-5 sm:p-6 shadow-xs space-y-4"
            >
              {/* Card Header */}
              <div className="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-slate-100">
                <div className="flex items-center gap-2">
                  <span className={`px-2.5 py-0.5 text-xs font-bold rounded-lg ${topic?.bgLight || 'bg-slate-100'}`}>
                    {topic?.shortTitle}
                  </span>
                ...
                <div className="bg-amber-50/70 border border-amber-200 rounded-xl p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <select
                      ...
                      className="text-xs border border-amber-300 rounded-lg p-1 bg-white"
                    >
                      <option value="need_review">Cần nghiên cứu gấp</option>
                      <option value="in_progress">Đang củng cố</option>
                      <option value="mastered">Đã thành thạo</option>
