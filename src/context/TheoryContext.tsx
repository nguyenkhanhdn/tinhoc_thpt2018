...
interface TheoryContextType {
  ...
  addLesson: (lesson: Omit<TheoryLesson, 'id' | 'updatedAt'>) => void;
  updateLesson: (id: string, lessonData: Partial<TheoryLesson>) => void;
...
export const TheoryProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [topics] = useState<SubjectTopic[]>(SUBJECT_TOPICS);
  ...
  const addLesson = (lessonData: Omit<TheoryLesson, 'id' | 'updatedAt'>) => {
  ...
  const updateLesson = (id: string, lessonData: Partial<TheoryLesson>) => {
